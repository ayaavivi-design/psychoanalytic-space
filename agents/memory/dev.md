...
## History (last 10)
1. feat(/hold): voice + i18n + design polish — Web Speech API (zero cost, `he-IL`/`en-US`), globe toggle + `bw_lang` localStorage, CSS vars throughout, `position:fixed;inset:0` fixes sidebar bleed from root layout. DB migration still pending. (מאי 2026)
