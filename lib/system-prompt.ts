import { THEORIST_VOICE, CORE_GUARDRAILS } from '@/lib/theorist-voices';

// ─────────────────────────────────────────────────────────────────────────────
// הרכבת הפרומפט הסטטי · חולץ מ-app/api/chat/route.ts ב-01.09.2026
//
// למה: מדד המובחנות חייב למדוד את הפרומפט של פרודקשן ולא העתק שלו. כל עוד
// ההרכבה חיה בתוך ה-handler, כל מודד חיצוני בונה שכפול, והשכפול נפרד מהמקור
// בשקט. זה בדיוק הכשל של הזיכרון הפרשני: נדון שבועות בזמן שלא הגיע למודל.
//
// הטקסט הועבר בהעתקה מדויקת ולא בשכתוב. אימות: `scripts/check-prompt-parity.mjs`
// משווה חתימות SHA של 90 צירופים (10 קולות × 3 מצבים × 3 מצבי שפה) מול
// `scripts/prompt-baseline.json` שנחתם לפני החילוץ.
// ─────────────────────────────────────────────────────────────────────────────

export interface StaticSystemOpts {
  theorist?: string;
  bw_mode?: string;
  uiLang?: string;
  persona?: string;
...
export function buildStaticSystem({ theorist, bw_mode, uiLang, persona }: StaticSystemOpts): string {
...
` : '';
  return (theorist && THEORIST_VOICE[theorist])
...
export function buildEndSessionSuffix(bw_end_session?: boolean): string {
...
- Speak as yourself. Not as a summarizer.` : '';
  return END_SESSION_SUFFIX;
}


// ─── בלוקי התחום · הועברו מ-route.ts ב-01.09.2026 ────────────────────────
// הם נקראים בזנב הדינמי ולא בבלוק הסטטי, ולכן אינם חלק מ-buildStaticSystem
// ואינם ב-baseline של בדיקת הזהות. הועברו לכאן כדי שמדד המובחנות יוכל
// לייבא אותם: קובץ route ב-Next אינו יכול לייצא ערכים שאינם handler.
export const CONSULT_SCOPE_INSTRUCTION = `
...
export const UNIVERSAL_SCOPE_INSTRUCTION = `
...
- YOU MAY DESCRIBE HOW THE FIELD HANDLES THIS, ONCE. Not a referral, not a name, not an instruction to go: the fact that therapists have colleagues, that practices have cover arrangements for exactly this, and that a shared clinic usually has someone who steps in. This is information about how her world works, and it is the one thing here that gives her a step she can take tomorrow without us choosing anyone for her. It also protects her therapist's place rather than replacing it. Say it once, plainly, and do not repeat it in later turns.
- ASK WHETHER SHE HAS ANYONE. Not a therapist — a friend, a partner, someone. She has lost the place that held her, and this is the only question here that changes anything tonight. It is not a referral and not treatment; it is checking that she is not alone with it. If she says yes, let that be enough and move on. If she says no, stay with that and do not turn it into a task.
