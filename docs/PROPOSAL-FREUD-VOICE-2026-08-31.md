...
> ## ✅ הושלם 31.08.2026 בערב · המסמך נשאר כתיעוד
> **חמשת השינויים בקוד ובפרודקשן.** כשבאתי ליישם התברר שהמסמך אינו מעודכן:
> **שינוי 1 · איחוד כללי הקצב** — כבר היה מיושם במלואו, וחמשת הכללים הישנים ירדו.
> **שינוי 4 · המבחן החיובי** — כבר היה מיושם במלואו.
> **שינוי 5 · הרגיסטר** — כבר היה מיושם במלואו.
> **שינוי 3 · הדוגמאות** — שתיים מתוך שלוש היו. `TOO EARLY TO NAME` נוספה עכשיו.
> **שינוי 2 · ההיסט בהצבעה לחדר** — לא היה, ונוסף עכשיו.
>
> **והלקח:** מסמך שמצהיר "לא מומש" בזמן שהקוד כבר השתנה **מסוכן כמו שני מקורות שכותבים לאותו אלמנט.** אישור עיוור שלו היה מיישם דברים שכבר מיושמים.
>
> **וגם:** שינוי 1 בוצע **ולא הספיק** — פרויד נכשל ב-`Q-3` ב-31.08 למרות שכלל הקצב המאוחד כבר היה בו, וזה מה שהוביל להוספת הצורה החד-משמעית של `16b` באותו ערב.
>
> **מה שנשאר פתוח ואינו שייך למסמך הזה:** אותה שורת "הצבעה חזרה לחדר" שתוקנה כאן קיימת בניסוחה הישן גם אצל **קליין, ויניקוט ואוגדן**. התיקון הוחל על פרויד בלבד, כפי שההצעה ביקשה.
...
**הבעיה.** **חמישה** כללים שולטים בציר אחד, כל אחד נכתב לתקן את קודמו. (בכתיבה הראשונה ספרתי ארבעה; סריקת ה-grep מצאה חמישי בשורה 48.)

| שורה | מה כתוב |
|---|---|
| [48](lib/theorist-voices.ts:48) | "שאלה אחת בלבד" בארבעת הכללים שלפני הכתיבה |
...
**מה יורד:** 48 · 179 · 188 · 443 · 444. **חמישה מקומות, בלוק אחד במקומם.**
**מה נכנס, במקום 188:**
...
## סריקת ה-grep · בוצעה 31.08, לפני כל עריכה

`AGENTS.md` דורש זאת לפני כל איסור: **לסרוק אחרי דוגמאות שמדגימות את ההפך**, כי "איסור בלי דוגמה, מול הדגמה עובדת של אותה התנהגות, מפסיד". הסריקה רצה על חמשת השינויים. **חינם, לא נגעה בקוד.**

| שינוי | סתירה? | מה נמצא |
|---|---|---|
| **1 · קצב** | **לא, אבל** | **חמישה כללים ולא ארבעה.** שורה 48 היא חמישית ולא נספרה |
| **2 · הצבעה חזרה** | לא | שלושת האתרים אושרו: 81 · 329 · 448. היסט, לא סתירה |
| **3 · דוגמאות** | לא, **ולטובה** | שני הכללים שהדוגמאות החדשות מדגימות **כבר קיימים כפרוזה** (אפקט חסר בשורה 121, "אני כבר יודע את זה" בשורה 315) **ואין להם דוגמה מעובדת.** כלומר זו אינה דוקטרינה חדשה, זו הדגמה לכלל קיים |
| **4 · 8c** | לא | מופע יחיד, אין דוגמה נגדית |
| **5 · רגיסטר** | **לא, ויש בעל ברית** | 8b כבר כותב "not softened into 'perhaps'". **הכלל שרציתי להוסיף כבר נתמך בקובץ**, וזה מוריד את הסיכון שרשמתי כבינוני |

**תיקון נוסף:** כתבתי "שלוש דוגמאות מעובדות". **יש ארבע**, כולל זוג ה-WRONG/RIGHT על סירוב בשורות 136-137.

**המשמעות: שער הבדיקה הראשון עבר לכל חמשת השינויים.** מה שנשאר הוא הכרעה, מימוש, וריצה חיה.

**ושינוי אחד בבלוק שקרה מאז הכתיבה:** טאב אחר הוסיף `WHEN SHE ASKS YOU TO DECIDE` בסוף הבלוק, והוא גדל מ-451 ל-463 שורות. **אינו מתנגש עם אף אחד מחמשת השינויים** (הוא יושב אחרי כל סעיפי הבדיקה הסופית).
